...
def cross_validation_GD(y, x, k_indices, k, gamma, max_iters):
    ...
    w_tr, mse_tr = least_squares_GD(y_tr, x_tr, w_initial, max_iters, gamma)
...
def cross_validation_ls(y, x, k_indices, k, degree):
    ...
    w_tr, mse_tr = least_squares(y_tr, res_tr)
    y_pred = predict_labels(w_tr, res_te)
    acc = np.mean(y_pred == y_te)
    return acc

def cross_validation_demo_ls(y, x, degrees):
    k_fold = 5
    seed = 1
    # split data in k fold
    k_indices = build_k_indices(y, k_fold, seed)
    ...
    for ind, degree in enumerate(degrees):
        ...
        ac = 0
        for k in range(k_fold):
            ac= cross_validation_ls(y, x, k_indices, k, degree)
            acc_m.append(ac)
        print("experience num {i} with degree = {j} and acc = {k}".format(i = ind, j = degree, k = np.mean(acc_m)) )
        acc.append(np.mean(acc_m))
        stds.append(np.std(acc_m))
